...
#define udf_error(...)   do {                   LOGE(stderr, "udfread ERROR: " __VA_ARGS__); } while (0)
#define udf_log(...)     do { if (enable_log)   LOGE(stderr, "udfread LOG  : " __VA_ARGS__); } while (0)
#define udf_trace(...)   do { if (enable_trace) LOGE(stderr, "udfread TRACE: " __VA_ARGS__); } while (0)
...
int udfread_open(udfread *udf, const char *path)
{
    ...
    if (!path) {
        LOGE("udfread_open fail error path %s",path);
        return -1;
    }

    input = block_input_new(path);
    if (!input) {
        LOGE("udfread_open fail block_input_new");
        return -1;
    }

    result = udfread_open_input(udf, input);
    if (result < 0) {
        LOGE("udfread_open fail udfread_open_input %d",result);
        if (input->close) {
            input->close(input);
        }
    }
...
UDFFILE *udfread_file_open(udfread *udf, const char *path)
{
    ...
    if (_find_file(udf, path, NULL, &fi) < 0) {
        LOGE("error opening _find_file fail\n");
        return NULL;
    }

    if (fi->characteristic & CHAR_FLAG_DIR) {
        LOGE("error opening file %s (is directory)\n", path);
        return NULL;
    }

    fe = _read_file_entry(udf, &fi->icb);
    if (!fe) {
        LOGE("error reading file entry for %s\n", path);
        return NULL;
    }

    result = (UDFFILE *)calloc(1, sizeof(UDFFILE));
    if (!result) {
        LOGE("error opening calloc fail\n");
        free_file_entry(&fe);
        return NULL;
    }
    result->udf = udf;
...
static ssize_t _read(UDFFILE *p, void *buf, size_t bytes)
{
    ...
    if (pos_off) {
        ...
        if (!p->block_valid) {
            if (udfread_read_blocks(p, p->block, file_block, 1, 0) != 1) {
                LOGE("udfread_read_blocks(p, p->block, file_block, 1, 0) != 1");
                return -1;
            }
            p->block_valid = 1;
        }
    ...
    if (bytes >= UDF_BLOCK_SIZE) {
        ...
        if (num_blocks < 1) {
            LOGE("num_blocks < 1 %u", num_blocks);
            return -1;
        }
...
ssize_t udfread_file_read(UDFFILE *p, void *buf, size_t bytes)
{
    ...
    /* sanity checks */
    if (!p || !buf) {
        LOGE("udfread_file_read, sanity checks error!");
        return -1;
    }
    if ((ssize_t)bytes < 0 || (int64_t)bytes < 0) {
        LOGE("udfread_file_read, bytes < 0 ");
        return -1;
    }
    ...
    if (!p->block) {
        ...
        if (!p->block_mem) {
            LOGE("udfread_file_read, !p->block_mem ");
            return -1;
        }
    ...
    while (bytes > 0) {
        ...
        if (r < 0) {
            ...
            /* got nothing */
            LOGE("udfread_file_read, got nothing");
            return -1;
        }
        bufpt += r;
        bytes -= (size_t)r;
